...
contract microFinance {
    ...
    function addFarmer(address farmer, string memory farmerId, string memory farmerName) public {
        farmerIdToAddressMapping[farmerId] = farmer;
        farmerMap[farmer] = Farmer(farmerId, farmerName);
        emit farmerAdded(farmer, farmerId);
    }
    
    function addProduce(
        string memory farmerId,
        string memory yieldId,
    ...
    function getDeal(string memory dealId) public view returns (
        string memory terms,
        uint256 amount,
        uint256 holdingPercent,
        address farmer
